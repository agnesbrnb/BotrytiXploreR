...
La méthode CART vise à construire un arbre de décision à partir des covariables. A chaque noeud construit est associé une condition dépendant des covariables. On construit dans un premier temps un arbre maximal à partir des données d'apprentissage puis on réalise une étape d'élagage pour réduire le nombre de feuilles. Les informations sur l'élagage optimale obtenues avec la fonction rpart sont utilisées pour effectuer l'élagage de l'arbre maximal. 
```{r}
...
On observe une erreur d'apprentissage de 47% pour l'arbre maximal et 36,7% pour l'arbre élagué avec les seed que nous avons utilisé. 
...
for(i in c(2018,1234,2017,1996,1107)) {
  ...
  data.rf = randomForest(label~., data=train, mtry=136,ntree=2000)
  data.rf
...
Il s'agit d'une méthode similaire au random forest mais on sélectionne toutes les covariables pour la construction de chaque noeud. Cette méthode se base sur le boostrap (ré-échantillonnage) pour générer ensuite un arbre maximal. Cette étape est répétée jsuqu'à atteindre la condition d'arrêt où les arbres seront moyennés. Cette méthode est améliore souvent la qualité de la prédiction. 
```{r}
error.bag <- numeric()
for(i in c(2018,1234,2017,1996,1107)) {
  set.seed(i)
