...
# for(i in c(2018,1234,2017,1996,1107)) {
  ...
  fit.svm = svm(label~., train[,c(select,ncol(train))], kernel="radial", probability=TRUE, cost=tune.out$best.parameters$cost, gamma=tune.out$best.parameters$gamma)
  mean(test$label != predict(fit.svm,newdata = test))
